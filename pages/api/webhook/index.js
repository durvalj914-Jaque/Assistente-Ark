...
async function handleOrder(db, msg, from, phoneNumberId) {
  ...
  const { data: contact } = await db.from('contacts').select('id,phone,full_name').eq('tenant_id', bot.tenant_id).eq('phone', from).maybeSingle()
...
async function processWebhook(body) {
  ...
  if (existingContact) {
    ...
    const { data: newContact, error: contactErr } = await db
      .from('contacts').insert({ tenant_id: tenantId, phone: from, full_name: contactName || null }).select('id,phone,full_name').single()
