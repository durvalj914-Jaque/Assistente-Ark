    ...
    interactive: {
      ...
      action: {
        buttons: buttons.slice(0,3).map((b,i) => ({
          type: 'reply',
          reply: { id: b.id || `btn_${i}`, title: (b.title || b.label || '').substring(0,20) }
        }))
      }
    }
  }, { headers: headers(token) })
...
export async function sendList(phoneId, token, to, body, sections) {
  // sections: [{ title: 'Opções', rows: [{ id, title, description }] }]
  // Max 10 rows total
  const totalRows = sections.reduce((sum, s) => sum + (s.rows?.length || 0), 0)
  if (totalRows > 10) sections = [{ title: 'Opções', rows: sections.flatMap(s => s.rows || []).slice(0, 10) }]
...
export async function sendImage(phoneId, token, to, imageUrl, caption = null) {
  const body = {
    messaging_product: 'whatsapp',
    to,
    type: 'image',
...
/**
 * Envia uma mensagem via template utility (hello_world como fallback).
 * Usado quando a janela 24h está fechada e o B2B tem créditos.
 * O webhook debita 1 crédito utility quando a Meta confirma a conversa.
 */
export async function sendUtilityTemplate(phoneId, token, to, templateName = 'hello_world', languageCode = 'en_US', params = []) {
  const template = {
    name: templateName,
    language: { code: languageCode },
  }
  // Se o template suporta variáveis, adicionar parâmetros
  if (params.length > 0) {
    template.components = [{
      type: 'body',
