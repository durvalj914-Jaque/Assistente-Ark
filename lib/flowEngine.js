...
export function resolveNext(nodes, current, userInput) {
  ...
  const isNumeric = !isNaN(num) && num > 0 && String(num) === userInput?.trim()
  ...
  if (current.buttons?.length) {
    // Match por número (1 = primeiro botão, 2 = segundo, etc.)
    if (isNumeric && num <= current.buttons.length) {
      const childId = current.children?.[num - 1]
  ...
  if (current.options?.length) {
    // Match por número (1 = primeira opção, etc.)
    if (isNumeric && num <= current.options.length) {
      const opt = current.options[num - 1]
      const targetId = opt.nextId || current.children?.[num - 1]
...
// action: 'root' | 'advance' | 'repeat' | 'transfer' | 'end' | 'catalog' | 'payment' | 'scheduling' | 'none'
export function processFlow(nodes, currentNodeId, userInput, { greeting } = {}) {
  ...
  if (next.action === 'scheduling') {
    return { reply: next.text, nodeId: next.id, action: 'scheduling', node: next }
  }
