...
export function resolveNext(nodes, current, userInput) {
  // Qualquer nó com opções (tipicamente tipo "menu", mas também aceito em
  // outros tipos por retrocompatibilidade com fluxos antigos) roteia pelo
  // keyword digitado — independente do "type" do nó.
  if (current.options?.length) {
    ...
    if (idx > -1) {
      ...
      // Prioridade: link explícito (opt.nextId) — usado em fluxos criados via SQL/migração.
      // Fallback: posição do ramo (children[idx]) — usado pelo Editor de Fluxo visual,
      // onde cada opção do menu corresponde, na mesma ordem, a um ramo criado com "+ ramo".
  ...
  // welcome / message / input / condition / transfer sem opções: avança pelo primeiro filho
...
export function processFlow(nodes, currentNodeId, userInput, { greeting } = {}) {
  ...
  if (next.type === 'catalog') {
    return { reply: next.text, nodeId: next.id, action: 'catalog', category: next.category || null }
  }

  if (next.type === 'payment') {
    return { reply: next.text, nodeId: next.id, action: 'payment', amount: next.amount || null, payMethod: next.payMethod || 'pix' }
  }
