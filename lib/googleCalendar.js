...
// Normaliza HH:MM (aceita '9:00', '9', Date serializado etc.)
function normTime(t) {
  if (!t) return '00:00'
...
// Normaliza data YYYY-MM-DD (caso venha como Date ISO completo)
function normDate(d) {
  if (!d) return null
...
export async function pushAppointmentToGoogle(db, tenantId, appt) {
  ...
  const evDate = normDate(appt.date)
  const evStart = normTime(appt.start_time)
  const evEnd = normTime(appt.end_time) || evStart
  const event = {
    ...
    start: { dateTime: `${evDate}T${evStart}:00${TZ}` },
    end:   { dateTime: `${evDate}T${evEnd}:00${TZ}` },
  }
  ...
  try {
    ...
    if (ev.id) {
      await db.from('appointments').update({
        ...
        gcal_link: ev.htmlLink || null,
        updated_at: new Date().toISOString(),
      }).eq('id', appt.id)
      console.log('[gcal] Evento criado:', ev.id, 'appt:', appt.id)
      return ev.id
    }
    console.error('[gcal] Google recusou o evento:', JSON.stringify(ev).substring(0, 300))
    return null
  } catch (e) {
