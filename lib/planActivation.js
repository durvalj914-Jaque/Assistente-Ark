...
export function parseResourceLimits(resourceNames = []) {
  ...
  for (const name of (resourceNames || [])) {
    ...
    // ACP — Ciclo de Comissão (ex: "Ciclo de Comissão (R$ 20)")
    const cycleMatch = lower.match(/(?:ciclo|comiss[^0-9]*r\$|threshold)[^0-9]*r?\$?\s*(\d+)/)
    if (cycleMatch && !lower.includes('pix') && !lower.includes('cart')) {
      const cyc = parseInt(cycleMatch[1])
      if (cyc >= 10) { limits.commission_cycle_threshold = Math.max(limits.commission_cycle_threshold || 0, cyc); continue }
    }
