...
export const PLANS = {
  ...
  starter: {
    label: 'Starter',
    price: 9700,
    commission_cycle_threshold: 20,
    max_bots: 3,
    max_messages_month: 5000,
    max_conversations_month: 500,
    google_product_id: 'ark_starter_monthly',
    features: ['3 bots', '500 conversas iniciadas/mês', 'Editor de fluxos', 'Analytics', 'Ciclo ACP de R$ 20,50 (2,43%)', 'Suporte prioritário']
  },
  pro: {
    label: 'Pro',
    price: 29700,
    commission_cycle_threshold: 30,
    max_bots: 10,
    max_messages_month: 25000,
    max_conversations_month: 2000,
    google_product_id: 'ark_pro_monthly',
    features: ['10 bots', '2.000 conversas iniciadas/mês', 'Fluxos avançados', 'Portal do cliente', 'Webhook + API', 'Ciclo ACP de R$ 30,50 (1,63%)', 'Suporte VIP']
  },
  enterprise: {
    label: 'Enterprise',
    price: null,
...
export function checkLimit(tenant, usage, botCount = 0, field) {
  ...
  if (sub && sub.status === 'active') {
    if (sub.expires_at && new Date(sub.expires_at) < new Date()) {
      ...
      if (field === 'messages' || field === 'conversations') return (usage?.business_initiated_conversations || 0) < (limits.max_conversations_month || limits.max_messages_month || 0)
  ...
  if (field === 'messages' || field === 'conversations') return (usage?.business_initiated_conversations || 0) < plan.max_conversations_month
...
export function usagePercent(tenant, usage) {
  ...
  if (sub && sub.status === 'active' && (!sub.expires_at || new Date(sub.expires_at) >= new Date())) {
    const max = sub.limits?.max_conversations_month || sub.limits?.max_messages_month || 0
    if (max >= 999999) return 0
    if (max > 0) return Math.min(Math.round(((usage?.business_initiated_conversations || 0) / max) * 100), 100)
    // subscription ativa SEM limits explícitos (ex: enterprise) → resolve pelo plano da subscription
    const subPlan = PLANS[sub?.plan]
    if (subPlan) {
      const subMax = subPlan.max_conversations_month
      if (subMax >= 999999) return 0
      if (subMax > 0) return Math.min(Math.round(((usage?.business_initiated_conversations || 0) / subMax) * 100), 100)
    }
  }

  // Fallback
  const plan = PLANS[tenant?.plan] || PLANS.free
  if (plan.max_conversations_month >= 999999) return 0
  return Math.min(Math.round(((usage?.business_initiated_conversations || 0) / plan.max_conversations_month) * 100), 100)
}
...
export function getEffectiveLimits(tenant) {
  ...
  if (sub && sub.status === 'active' && (!sub.expires_at || new Date(sub.expires_at) >= new Date())) {
    return {
      ...
      max_conversations_month: sub.limits?.max_conversations_month || 50,
  ...
  return {
    ...
    max_conversations_month: plan.max_conversations_month,
